...
def equirectangular_to_cubemap(equi_img):
    """
    Convert an equirectangular image into two cubemap faces (right and left).
    """
    face_size = CUBE_FACE_SIZE
    faces = {}

    # Define angles for the left and right faces
    phi_angles = {
        'left': -np.pi / 2,  # Left face at -90 degrees
        'right': np.pi / 2   # Right face at 90 degrees
    }
    
    # Generate left and right faces only
...
def save_cubemap_faces(cubemap_faces, output_dir, image_name):
    """
    Saves only the 'left' and 'right' cubemap faces as JPEG images.
    """
...
def process_equirectangular_images(input_dir, output_dir):
    """
    Main function to process each equirectangular image into left and right cubemap faces.
    """
