...
def run_colmap(db_path, images_path, text_path, colmap_camera_model="SIMPLE_PINHOLE", colmap_matcher="sequential", colmap_camera_params="", overwrite=True):
	...
	if not overwrite and (input(f"warning! folders '{sparse}' and '{text}' will be deleted/replaced. continue? (Y/n)").lower().strip()+"y")[:1] != "y":
		sys.exit(1)
...
if __name__ == "__main__":
	...
	# OUT_PATH = args.out
	main(
		db_path="colmap_output/colmap.db",
		img_path="images",
		text_path="colmap_output\colmap_text",
		output_path="colmap_output\\transforms.json",
		aabb_scale=1
	)
